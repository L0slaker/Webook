...
func NewSMSService(svc sms.Service, repo repository.AsyncSMSRepository, l logger.LoggerV1) *SMSService {
	res := &SMSService{
		svc:  svc,
		repo: repo,
		l:    l,
...
// StartAsyncCycle 异步发送消息，这里我们没有设计退出机制，是因为没啥必要
// 因为程序停止的时候，它自然就停止了；原理：这是最简单的抢占式调度
...
func (s *SMSService) AsyncSend() {
	...
	default:
		// 数据库那边出了问题，但是为了尽量运行，还是要继续的
		// 可以稍微休眠，也可以不休眠，休眠可以帮忙规避掉短时间的网络抖动问题
